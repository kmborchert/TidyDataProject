...
Seven files from the .zip file are needed for the analysis. 
* Numerical data for training and test sets reside in the files "X_train.txt" and "X_test.txt".  
* The column headings for the numerical data reside in the file "features.txt".  
* Subject IDs for each of the rows of numerical data reside in the files "subject_train.txt" and "subject_test.txt".
* Activity codes for each row of numerical data reside within "y_train.txt" and "y_test.txt".
* The file "activity_labels.txt" contains names for each of the activity codes (a key, if you will).

####FILES USED TO CREATE THE TIDY DATA
1) The R script called "run_analysis.R", located within my Git Respository
(https://github.com/kmborchert/TidyDataProject.git) reads in data files described below
which have been downloaded into the /data subfolder of the users working directory.
